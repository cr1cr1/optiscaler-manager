...
func ResetInputSession() {
	...
	// PATCHED by optiscaler-manager (v0.16): reset the identity tree so
	// back-to-back headless runs (tests, RenderToImage) don't leak widget
	// state — virtual-list scroll/height caches, Use hooks — across runs.
	// All ResetInputSession callers are headless; live backends never call it.
	// Reapply after `go mod vendor` (see docs/vendor-patches.md).
